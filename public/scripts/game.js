var game = {
	...
	tick: function () {
		...
		if ((game.ticks - game._wave) % 30 === 29) {
			ui.timer.style.width = (((game.ticks - game._wave) / 60) * 0.05) * 100 + "px";
		}
		...
		game.creeps.forEach(function (creep, i, a) {
			...
			if (creep.hp <= 0) {
				...
				var creepImages = [];
				// creepImages[0] = new Image();
				// creepImages[0].src = 'images/creeps/kidaha-01.svg';

				// creepImages[1] = new Image();
				// creepImages[1].src = 'images/creeps/kidaha-02.svg';
				for( i=1; i<=12; i++){
					creepImages[i] = new Image();
					creepImages[i].src = 'images/creeps/kidaha-0'+i+'.svg';
				}
				
				canvas.fillStyle = "hsl(" + hue + "," + sat + "%,50%)";
				// canvas.fillRect(creep.x - 5, creep.y - 5, 10, 10);
				var creepImage = creepImages[1];
				if( creep.hp > 100 ){
					var creepImage = creepImages[2];
				}
				if( creep.hp > 200 ){
					var creepImage = creepImages[3];
				}
				if( creep.hp > 300 ){
					var creepImage = creepImages[4];
				}
				if( creep.hp > 400 ){
					var creepImage = creepImages[5];
				}
				if( creep.hp > 500 ){
					var creepImage = creepImages[6];
				}
				if( creep.hp > 600 ){
					var creepImage = creepImages[7];
				}
				if( creep.hp > 700 ){
					var creepImage = creepImages[8];
				}
				if( creep.hp > 800 ){
					var creepImage = creepImages[9];
				}
				if( creep.hp > 900 ){
					var creepImage = creepImages[10];
				}
				if( creep.hp > 1000 ){
					var creepImage = creepImages[11];
				}
				if( creep.hp > 1500 ){
					var creepImage = creepImages[12];
				}
				if (creepImage.complete) { // Check if the image has loaded
					// Draw the creep image at its position
					canvas.drawImage(creepImage, creep.x - 12.5, creep.y - 12.5, 25, 25);
				}
			}
		});
		...
		game.turrets.forEach(function (turret) {
			...
			canvas.drawImage(turret.img, turret.x - 12.5, turret.y - 12.5, 25, 25);
		});
	...
	pause: function () {
		...
		game.turrets.sort(function (a, b) {
			return b.kills - a.kills;
		});

		console.log(game.turrets);
